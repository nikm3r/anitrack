...
// ─── MAL OAuth local callback server ──────────────────────────────────────────
// Spins up a temporary HTTP server on port 54321 during the auth flow.
// MAL redirects to http://localhost:54321/callback?code=... after the user logs in.
// We catch the code, send it to the renderer, then shut the server down.

let _malAuthServer = null;

ipcMain.handle("mal:start-auth", async (_e, { clientId, codeVerifier }) => {
  // If a server is already running from a previous attempt, close it first
  if (_malAuthServer) {
    try { _malAuthServer.close(); } catch {}
    _malAuthServer = null;
  }
  ...
  return new Promise((resolve) => {
    const server = http.createServer((req, res) => {
      try {
        ...
        if (code) {
          ...
          res.end(`<!DOCTYPE html><html><head><title>AniTrack</title>
            <style>body{font-family:system-ui;background:#0a0a0f;color:#fff;display:flex;
            align-items:center;justify-content:center;height:100vh;margin:0;}
            .box{text-align:center;padding:2rem;}h1{color:#10b981;}</style></head>
            <body><div class="box"><h1>&#10003; Authorised!</h1>
            <p>You can close this tab and return to AniTrack.</p></div></body></html>`);

          // Send code back to renderer
          mainWindow?.webContents.send("mal:auth-code", { code });
          resolve({ ok: true });
        } else {
          res.writeHead(400, { "Content-Type": "text/html" });
          res.end(`<!DOCTYPE html><html><body>
            <p>Auth failed: ${error || "No code received"}</p></body></html>`);
          resolve({ ok: false, error: error || "No code in callback" });
        }
      } catch (err) {
        res.writeHead(500);
        ...
        // Shut down after handling one request
        server.close();
        _malAuthServer = null;
      }
    });
    ...
    server.on("error", (err) => {
      console.error("[mal-auth] Server error:", err.message);
      resolve({ ok: false, error: err.message });
    });

    server.listen(port, "127.0.0.1", () => {
      _malAuthServer = server;
      console.log(`[mal-auth] Listening on port ${port}`);

      // Build the auth URL and open it in the browser
    ...
    // Timeout after 5 minutes — user probably closed the browser
    setTimeout(() => {
      if (_malAuthServer) {
        _malAuthServer.close();
        _malAuthServer = null;
        resolve({ ok: false, error: "Auth timed out" });
      }
    }, 5 * 60 * 1000);
  });
});
