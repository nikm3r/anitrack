...
export class AniListTracker implements ITracker {
    ...
    for (const list of listData.MediaListCollection.lists) {
      for (const e of list.entries) {
        entries.push({
          ...
          // AniList returns POINT_100 (0–100); normalize to internal 0–10
  ...
  async updateProgress(token: string, mediaId: string, progress: number, status: string): Promise<void> {
    ...
    await gql(mutation, { mediaId: parseInt(mediaId, 10), progress, status: anilistStatus }, token);
  ...
  async updateScore(token: string, mediaId: string, score: number): Promise<void> {
    // Internal score is always 0–10. AniList's SaveMediaListEntry accepts
    // a float and respects the user's own scoring format server-side.
    const mutation = `
    ...
    `;
    await gql(mutation, { mediaId: parseInt(mediaId, 10), score }, token);
    console.log(`[anilist] Updated score for media ${mediaId}: ${score}`);
  }

  // ─── AniList-specific: season browse ─────────────────────────────────────────
  // Not part of ITracker — only called directly from browse.ts which is AniList-only for now.
