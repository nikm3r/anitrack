...
function guessEpisode(filename: string, totalEpisodes?: number | null): number | null {
  ...
  const tryN = (n: number): number | null => {
    if (n <= 0) return null;
    // FIX: Exclude year-like numbers (1900–2100) so filenames like
    //      "Anime.2024.mkv" don't get parsed as episode 2024.
...
function findPlayer(): {
  exe: string;
  args: (filePath: string, forSync: boolean) => string[];
  type: "mpv" | "vlc";
  vlcPort?: number;  // FIX: expose the port directly instead of re-parsing from args
} | null {
  const db = getDb();
  ...
  const ordered = playerMode === "vlc"
    ? [
        { paths: vlcCandidates, args: vlcArgs, type: "vlc" as const, vlcPort },
  ...
  for (const candidate of ordered) {
    for (const p of candidate.paths) {
      if (path.isAbsolute(p)) {
        if (fs.existsSync(p)) return { exe: p, args: candidate.args, type: candidate.type, vlcPort: candidate.vlcPort };
      } else {
        return { exe: p, args: candidate.args, type: candidate.type, vlcPort: candidate.vlcPort };
      }
    }
  }
  return null;
